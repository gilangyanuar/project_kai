...
@section('content')
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-8 pb-4 mb-10">
        ...
        <div class="space-y-4 mb-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                <label class="font-bold text-gray-800">No. Seri Pembangkitan</label>
                <div class="md:col-span-3">
                    <input type="text" value="33" class="w-full rounded-md border border-gray-300 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                <label class="font-bold text-gray-800">Tipe Genset</label>
                <div class="md:col-span-3">
                    <input type="text" value="Diesel" class="w-full rounded-md border border-gray-300 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                ...
                <div class="md:col-span-3 grid grid-cols-2 gap-4">
                    <input type="text" value="NIPP.333211213" class="w-full rounded-md border border-gray-300 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <input type="text" value="NIPP.422411134" class="w-full rounded-md border border-gray-300 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
        ...
        <div class="overflow-x-auto pb-2">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm text-gray-700">
                    @php
                        $items = [
                            ['no' => 1, 'suku' => 'Bocor/Tetesan Pelumas', 'standar' => 'Baik', 'hasil' => 'Tidak Ada', 'petugas' => 'Adi', 'ket' => ''],
                            ['no' => 2, 'suku' => 'HSD di dalam pipa', 'standar' => 'Berfungsi', 'hasil' => 'Baik', 'petugas' => 'Rafael', 'ket' => ''],
                            ['no' => 3, 'suku' => 'Pelumas pada deep stiek', 'standar' => 'Cukup', 'hasil' => 'Cukup', 'petugas' => 'Ali', 'ket' => ''],
                        ];
                    @endphp
                    ...
                    <tr class="border-b border-gray-100 hover:bg-gray-50 {{ $index % 2 != 0 ? 'bg-surface-alt' : 'bg-white' }}">
                        ...
                        <td class="px-4 py-3 text-center" x-data="{ open: false, selected: '{{ $item['standar'] }}' }">
                            <button x-ref="btnStd" @click="open = !open" type="button"
                                ...
                                :class="{
                                    'bg-green-400 text-white': selected === 'Baik',
                                    'bg-blue-300 text-blue-900': selected === 'Berfungsi',
                                    'bg-orange-200 text-orange-800': selected === 'Cukup'
                                }">
                            ...
                            <template x-teleport="body">
                                <div x-show="open" 
                                    ...
                                    <div class="space-y-1">
                                        @foreach(['Baik', 'Berfungsi', 'Cukup'] as $opt)
                                            @php
                                                // Logic Warna Badge di dalam Dropdown
                                                $bgClass = match($opt) {
                                                    'Baik' => 'bg-green-400',
                                                    'Berfungsi' => 'bg-blue-300',
                                                    'Cukup' => 'bg-orange-200',
                                                };
                                                // Logic Warna Teks (Putih untuk bg gelap, Gelap untuk bg terang)
                                                $textClass = $opt === 'Baik' ? 'text-white' : ($opt === 'Berfungsi' ? 'text-blue-900' : 'text-orange-800');
                                            @endphp
                                            ...
                                            <div @click="selected = '{{ $opt }}'; open = false" class="flex items-center gap-2 p-2 hover:bg-gray-100 cursor-pointer rounded">
                                                ...
                                                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold {{ $bgClass }} {{ $textClass }} w-full text-center">
                                                    {{ $opt }}
                                                </span>
                                            </div>
                                        @endforeach
                        ...
                        <td class="px-4 py-3 text-center" x-data="{ open: false, selected: '{{ $item['hasil'] }}' }">
                            <button x-ref="btnHasil" @click="open = !open" type="button"
                                ...
                                :class="{
                                    ...
                                    'bg-green-400 text-white': selected === 'Baik',
                                    'bg-yellow-300 text-yellow-900': selected === 'Cukup' || selected === 'Ada'
                                }">
                                <span x-text="selected"></span>
                                <span class="material-symbols-outlined text-[14px]">edit</span>
                            </button>
                            ...
                            <template x-teleport="body">
                                <div x-show="open" 
                                    ...
                                    <div class="space-y-1">
                                        @foreach(['Tidak Ada', 'Ada', 'Baik', 'Cukup'] as $opt)
                                            @php
                                                $bgClass = match($opt) {
                                                    'Tidak Ada' => 'bg-gray-500',
                                                    'Ada', 'Cukup' => 'bg-yellow-300 text-yellow-900',
                                                    'Baik' => 'bg-green-400',
                                                    default => 'bg-gray-200'
                                                };
                                                $textClass = str_contains($bgClass, 'text-') ? '' : 'text-white';
                                            @endphp
                                            <div @click="selected = '{{ $opt }}'; open = false" class="flex items-center gap-2 p-2 hover:bg-gray-100 cursor-pointer rounded">
                                                <div class="w-5 h-5 border border-gray-300 rounded flex items-center justify-center"
                                                    :class="selected === '{{ $opt }}' ? 'bg-kai-navy border-kai-navy' : ''">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('master.genset.edit') }}" class="text-blue-600 hover:text-blue-800"><span class="material-symbols-outlined text-[20px]">edit_square</span></a>
